...
namespace TcNo_Acc_Switcher_Globals
{
    internal static class NativeMethods
    {
        ...
        // KrDv-Hes-Degistir-Client NativeMethods
    ...
    public static class NativeFuncs
    {
        ...
        public static bool BringToFront()
        {
            ...
            var proc = Process.GetProcessesByName("KrDv-Hes-Degistir_main").FirstOrDefault();
        ...
        {
            if (Process.GetProcessesByName("KrDv-Hes-Degistir-Tray_main").Length > 0) return "Already running";
            if (!File.Exists("Tray_Users.json")) return "Tray users not found";
            var startInfo = new ProcessStartInfo { FileName = Path.Join(Globals.AppDataFolder, "KrDv-Hes-Degistir-Tray.exe"), CreateNoWindow = false, UseShellExecute = false, WorkingDirectory = Globals.AppDataFolder };
            try
            {
