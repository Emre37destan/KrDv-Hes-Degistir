// KrDv Hesap Degistir - A Super fast account switcher
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
        ...
        private void LaunchAccSwitcher(object sender, RoutedEventArgs e)
        {
            try
            {
                _ = Process.Start(new ProcessStartInfo(@"KrDv-Hes-Degistir.exe") { UseShellExecute = true });
                Process.GetCurrentProcess().Kill();
            }
            catch (Win32Exception we)
            {
                MessageBox.Show(
                    $"An error occurred while trying to start. Patching may have failed. Try verify using this updater, or re-download a fresh copy from the KrDv Hesap Degistir GitHub.{Environment.NewLine}{we}",
                    "Could not start KrDv Hesap Degistir", MessageBoxButton.OK, MessageBoxImage.Error);


                SetStatus("An error occurred while trying to start. Patching may have failed. Try verify using this updater, or re-download a fresh copy from the KrDv Hesap Degistir GitHub.");
        ...
        public static void RestartAsAdmin(string args)
        {
            var proc = new ProcessStartInfo
            {
                ...
                FileName = Assembly.GetEntryAssembly()?.Location.Replace(".dll", ".exe") ?? "KrDv-Hes-Degistir.exe",
        ...
        private static string GetVersionFromGlobals()
        {
            ...
            var dllPath = Path.Join(Directory.GetCurrentDirectory(), "KrDv-Hes-Degistir-Globals.dll");
        ...
        {
            const string newFolder = "KrDv-Hes-Degistir";
        ...
        // FOR TESTING: public static string MainAppDataFolder => "C:\\Program Files\\KrDv Hesap Degistir";
        ...
        private void DoUpdate(bool cef = false)
        {
            ...
            SetStatusAndLog("Closing KrDv Hesap Degistir instances (if any).");
            ...
            {
                var downloadUrl = "";
                if (cef) downloadUrl = $"https://github.com/Emre37destan/KrDv-Hes-Degistir/releases/download/{_latestVersion}/KrDv-Hes-Degistir_and_CEF_{_latestVersion}.7z";
                else downloadUrl = $"https://github.com/Emre37destan/KrDv-Hes-Degistir/releases/download/{_latestVersion}/KrDv-Hes-Degistir_{_latestVersion}.7z";
            ...
            var tempFolder = Path.Combine(Path.GetTempPath(), "KrDv-Hes-Degistir");
        ...
        private static bool usingCEF()
        {
            var path = File.Exists(Path.Join(AppDataFolder, "userdata_path.txt"))
                ...
                : Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "KrDv Hesap Degistir\\");
        ...
        /// <summary>
        /// Closes KrDv Hesap Degistir's processes if any are running. (Searches program's folder recursively)
        /// </summary>
        ...
        private void CloseIfRunning(string currentDir)
        {
            foreach (var exe in Directory.GetFiles(currentDir, "*.exe", SearchOption.AllDirectories))
            {
                ...
                try
                {
                    if (Process.GetProcessesByName(exe).Length <= 0 || exe == "KrDv-Hes-Degistir-Updater.exe" || exe == "KrDv-Hes-Degistir-Updater_main.exe") continue;
