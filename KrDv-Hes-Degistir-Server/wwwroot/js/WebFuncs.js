...
async function getCurrentPageFullname() {
    ...
    if (platform === "Basic") {
        platform = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiCurrentBasicPlatform", platform);
    }
    return platform;
}
...
GetLang = async(k) => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiLocale", k);
GetCrowdin = async() => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiCrowdinList");
GetLangSub = async(key, obj) => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiLocaleObj", key, obj);

copyToClipboard = async(str) => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "CopyToClipboard", str);

// FORGETTING ACCOUNTS
async function forget(e) {
    e.preventDefault();
    const reqId = $(selectedElem).attr("id");
    const pageForgetAcc = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `Get${getCurrentPage()}ForgetAcc`);
...
getAccNotes = async(accId) => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `Get${getCurrentPage()}Notes`, accId);
...
async function copy(request, e) {
    ...
    async function steam() {
        ...
        switch (request) {
            ...
            case "SteamId3":
            case "SteamId":
                await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "CopySteamIdType", request, steamId64);
                break;
...
async function swapTo(request, e) {
    ...
    if (request === -1) await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `SwapTo${getCurrentPage()}`, selected.attr("id")); // -1 is for undefined.
    else await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `SwapTo${getCurrentPage()}WithReq`, selected.attr("id"), request);
}
...
async function CopySettingsFrom(e, game) {
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "CopySettingsFrom", steamId64, game);
}
...
async function RestoreSettingsTo(e, game) {
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "RestoreSettingsTo", steamId64, game);
}
...
async function BackupGameData(e, game) {
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "BackupGameData", steamId64, game);
}
...
async function ShowGameStatsSetup(e = null) {
    ...
    const enabledGames =
        await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GetEnabledGames", currentPage, accountId);
    const disabledGames = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GetDisabledGames", currentPage, accountId);
...
async function toggleGameStats(safeGame, isChecked) {
    ...
    if (!isChecked) {
        ...
        await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `DisableGame`, game, accountId);
        ShowGameStatsSetup();
        return;
    }
...
async function showGameStatsVars(game) {
    ...
    // Checked: Get required variables and present to user.
    const required = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `GetRequiredVars`, game);
    const existing = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `GetExistingVars`, game, accountId);
    const hidden = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `GetHiddenMetrics`, game, accountId);
    const globallyHidden = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `GetGloballyHiddenMetrics`, game);
...
async function Modal_FinaliseGameVars(game, accountId) {
    ...
    const requiredVars = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `GetRequiredVars`, game);
    const possibleHidden = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `GetHiddenMetrics`, game, accountId);
    const globallyHidden = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `GetGloballyHiddenMetrics`, game);
    ...
    $(".modalBG").fadeOut();
    const success = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `SetGameVars`, currentPage, game, accountId, returnDict, hidden);
...
async function refreshAccount(game, accountId) {
    const currentPage = await getCurrentPageFullname();
    $(".modalBG").fadeOut();
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `RefreshAccount`, accountId, game, currentPage);
    location.reload();
}



Modal_FinalizeImage = async(dest) => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `ImportNewImage`, JSON.stringify({ dest: dest, path: $("#FolderLocation").val() }));
...
async function openUserdata(e) {
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `SteamOpenUserdata`, selected.attr("id"));
}
...
async function createShortcut(args = '') {
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server",
        "CreateShortcut",
        getCurrentPage(),
        accId,
        getDisplayName(),
...
// NEW LOGIN
newLogin = async() => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `NewLogin_${getCurrentPage()}`);
hidePlatform = async() => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "HidePlatform", selectedElem);
createPlatformShortcut = async() => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiCreatePlatformShortcut", selectedElem);
...
async function exportAllAccounts() {
    ...
    const r = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiExportAccountList", selectedElem);
...
// Link handling
OpenLinkInBrowser = async(link) => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "OpenLinkInBrowser", link);
...
async function showModal(modaltype) {
    ...
    if (modaltype === "info") {
        ...
        const currentVersion = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiGetVersion");
        $("#modal_contents").append(`<div class="infoWindow">
                ...
                <div class="rightContent">
                    ...
                    <div class="linksList">
                        ...
                        <a onclick="OpenLinkInBrowser('https://github.com/Emre37destan/KrDv-Hes-Degistir');return false;" href=""><svg viewBox="0 0 24 24" draggable="false" alt="GitHub" class="modalIcoGitHub"><use href="img/icons/ico_github.svg#icoGitHub"></use></svg>${modalInfoViewGitHub}</a>
                        ...
                        <a onclick="OpenLinkInBrowser('https://github.com/Emre37destan/KrDv-Hes-Degistir/blob/master/DISCLAIMER.md');return false;" href=""><svg viewBox="0 0 2084 2084" draggable="false" alt="GitHub" class="modalIcoDoc"><use href="img/icons/ico_doc.svg#icoDoc"></use></svg>${modalInfoDisclaimer}</a>
                    </div>
                </div>
        ...
        if (platformName === "Basic") {
            extraButtons = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "PlatformUserModalExtraButtons");
        ...
        const modalChangeUsername = await GetLang("Modal_SetPassword"),
            ...
            modalSetPasswordInfo = await GetLangSub("Modal_SetPassword_Info", { link: "https://github.com/Emre37destan/KrDv-Hes-Degistir/wiki/FAQ---More-Info#can-i-put-this-program-on-a-usb-portable" });
        ...
        const currentBasicPlatformExe =
            await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiCurrentBasicPlatformExe", platform);
        ...
        const extraButtons = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "PlatformUserModalExtraButtons");
        ...
        const modalText = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GetStatsString");
...
async function getLogicalDrives() {
    var folderContent = "";
    const logicalDrives = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GetLogicalDrives");
    folderContent = "<div>";
...
async function pathPickerClick(e) {
    ...
    if ($(e.target).hasClass("folder") && !$(e.target).hasClass("c")) {
        ...
        const fileSystemResult = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", getFunc, result);
...
//    DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiUpdatePath", platformSettingsPath, $("#FolderLocation").val());
//    $(".modalBG").fadeOut();
...
async function Modal_Finalise(platform, platformSettingsPath) {
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiUpdatePath", platformSettingsPath, $("#FolderLocation").val());
    $(".modalBG").fadeOut();
...
async function Modal_Confirm(action, value) {
    ...
    const confirmAction = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiConfirmAction", action, value);
...
async function Modal_FinaliseAccString(platform) {
    ...
    if (raw.indexOf(":{") === -1) {
        name = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiGetCleanFilePath", raw);
    }
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", platform + "AddCurrent", name);
    $(".modalBG").fadeOut();
    $("#acc_list").click();
}
...
async function Modal_FinaliseBackground() {
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "SetBackground", pathOrUrl);
    $(".modalBG").fadeOut();
}

async function Modal_FinaliseSwitcherPassword() {
    const switcherPassword = $("#SwitcherPassword").val();
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "SetSwitcherPassword", switcherPassword);
    $(".modalBG").fadeOut();
}
...
async function Modal_FinaliseUserDataFolder() {
    ...
    const pathOrUrl = $("#FolderLocation").val();
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "SetUserData", pathOrUrl);
    $(".modalBG").fadeOut();
}
...
async function Modal_FinaliseAccNameChange() {
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "ChangeUsername", $(".acc:checked").attr("id"), name);
}

async function Modal_SaveNotes(accId) {
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", `Set${getCurrentPage()}Notes`, accId, $('#accNotes').val());
    location.reload(true);
}
...
async function usernameModalCopyText() {
    ...
    const toastHintText = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "PlatformHintText", platform);
    const code = await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "PlatformUserModalCopyText");

    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "CopyToClipboard", code);
    window.notification.new({
	    type: "success",
        title: toastTitle,
...
async function serializeShortcuts() {
    ...
    if (getCurrentPage() === "Steam")
        await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "SaveShortcutOrderSteam", output);
    else
        await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "SaveShortcutOrder", output);
}
...
async function shortcut(action) {
    ...
    console.log(reqId);
    if (getCurrentPage() === "Steam")
        await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "HandleShortcutActionSteam", reqId, action);
    else
        await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "HandleShortcutAction", reqId, action);
    if (action === "hide") $(selectedElem).remove();
}

updateBarClick = async () => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "UpdateNow");

async function initSavingHotKey() {
    hotkeys("ctrl+s", async function (event) {
        event.preventDefault();
        await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiCtrlS", getCurrentPage());
    });
}
...
restartAsAdmin = async(args = "") => await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiRestartAsAdmin", args);