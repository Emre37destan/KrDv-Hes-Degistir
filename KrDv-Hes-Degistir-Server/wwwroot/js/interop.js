...
async function initPlatformListSortable() {
    ...
    sortable(".platform_list")[0].addEventListener("sortupdate", () => {
        ...
        DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiSaveOrder", `Settings\\platformOrder.json`, JSON.stringify(order));
    });
}
...
async function initAccListSortable() {
    ...
    await DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiCurrentBasicPlatform", platformName).then((r) => {
        platformName = r;
    });
    ...
    sortable(".acc_list")[0].addEventListener("sortupdate", (e) => {
        ...
        DotNet.invokeMethodAsync("KrDv-Hes-Degistir-Server", "GiSaveOrder", `LoginCache\\${platformName}\\order.json`, JSON.stringify(order));
    });
}
