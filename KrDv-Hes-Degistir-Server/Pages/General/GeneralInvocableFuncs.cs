...
namespace TcNo_Acc_Switcher_Server.Pages.General
{
    public class GeneralInvocableFuncs
    {
        ...
        {
            var fileName = "KrDv-Hes-Degistir_main.exe";
            if (!AppData.TcNoClientApp) fileName = Assembly.GetEntryAssembly()?.Location.Replace(".dll", ".exe") ?? "KrDv-Hes-Degistir-Server_main.exe";
            else
            {
                // Is client app, but could be developing >> No _main just yet.
                if (!File.Exists(Path.Join(Globals.AppDataFolder, fileName)) && File.Exists(Path.Join(Globals.AppDataFolder, "KrDv-Hes-Degistir.exe")))
                    fileName = Path.Combine(Globals.AppDataFolder, "KrDv-Hes-Degistir.exe");
            }
        ...
        public static void CreateShortcut(string page, string accId, string accName, string args = "")
        {
            ...
            _ = s.Shortcut_Platform(
                ...
                $"Switch to {accName} [{platform}] in KrDv Hesap Degistir",
                true);
