...
namespace TcNo_Acc_Switcher_Server.Data
{
    public class AppSettings
    {
        ...
        public static void CheckShortcuts()
        {
            ...
            DesktopShortcut = File.Exists(Path.Join(Shortcut.Desktop, "KrDv Hesap Degistir.lnk"));
            StartMenu = File.Exists(Path.Join(Shortcut.StartMenu, "KrDv Hesap Degistir.lnk"));
        ...
        public static void Protocol_Toggle()
        {
            try
            {
                if (!Protocol_IsEnabled())
                {
                    ...
                    defaultKey?.SetValue("", $"\"{Path.Join(Globals.AppDataFolder, "KrDv-Hes-Degistir.exe")}\" \"%1\"", RegistryValueKind.String);
        ...
        public static void StartUpdaterAsAdmin(string args = "")
        {
            ...
            var proc = new ProcessStartInfo
            {
                ...
                FileName = "updater\\KrDv-Hes-Degistir-Updater.exe",
                Arguments = args,
        ...
        public static void AutoStartUpdaterAsAdmin(string args = "")
        {
            ...
            {
                _ = Process.Start(new ProcessStartInfo(Path.Join(Globals.AppDataFolder, @"updater\\KrDv-Hes-Degistir-Updater.exe")) { UseShellExecute = true, Arguments = args });
                try
                {
                    AppData.ActiveNavMan.NavigateTo("EXIT_APP", true);
                }
